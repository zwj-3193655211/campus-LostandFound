...
| 类别 | 命令 | 结果 | 备注 |
|---|---|---|---|
| 后端单元/集成测试 | `mvn test` | 通过 | 共 64 个测试，13 个测试文件，2 跳过 |
...
| `AuthServiceTest` | 单元测试 | 注册/登录核心服务（含 ON/OFF 两种模式 + 邮箱验证码集成）|
| `ItemServiceImplTest` | 单元测试 | 物品状态门禁 + 管理员审核(原子更新、并发竞态、通知异常兜底) |
...
- 已新增 `ItemServiceImplTest`，覆盖管理员物品审核的状态机门禁（仅 PENDING 可审）、按 `id+status=PENDING` 条件原子更新的并发竞态拦截、通知发送异常不影响主流程、物品不存在等关键路径，共 6 个测试用例。
- 已补 `AdminItemReviewController` 的端到端 E2E 验证（12 用例，含普通用户 403、campusadmin/superadmin 双角色、状态机门禁、分页查询、Swagger 可见性）。
- 已补邮箱验证码注册的全链路 E2E：OFF 模式 11/11 用例（冷却、取码、正确码注册、登录、错误码拒绝、过期码拒绝、邮箱查重、大小写归一化）+ ON 模式 7/7 用例（不带码拒绝、错误码拒绝、复用拒绝、邮箱查重）。

## 6. 当前质量风险

### 6.1 高优先级
- （已收敛）管理员物品审核的缺参组合、状态机门禁与 MySQL 实际并发场景已由 `ItemServiceImplTest`（6 用例）覆盖；后续若新增物品状态字段需补对应迁移。
...
## 9. 当前结论
- 后端自动化测试已具备基础回归能力（**64 测试 / 13 文件**），权限矩阵、JWT 鉴权链路、状态机门禁、并发竞态拦截、通知异常兜底均已覆盖。
- 前端已具备 Vitest 基线，但仍需要把测试从“守卫/拦截器”扩展到“登录弹窗、管理员页面、发布页”等高价值交互。
- 2026-06-07 第十八轮已补全：管理员物品审核的 `PUT /api/admin/items/{id}/review` 真实接口（之前 Swagger 中不存在导致物品永远卡 `PENDING`）、邮箱验证码注册 UI 接入与开关控制。
- 如果下一阶段继续提升交付质量，优先级应是“扩展前端测试覆盖”和“补 MyBatis-Plus 软删除相关 count 查询的一致性测试”。
