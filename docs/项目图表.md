...
17. [ER 关系图](#17-er-关系图)
18. [匹配状态机](#18-匹配状态机)
19. [实名认证状态机](#19-实名认证状态机)
20. [完成申请状态机](#20-完成申请状态机)
...
> v3 算法（2026-06 升级）：使用 Jieba 中文分词库，支持智能中文分词，阈值调整为 70%。
...
- `items.status` 状态枚举：`PENDING` → `APPROVED` → `REJECTED` / `WITHDRAWN` / `COMPLETED`（详见 §8 状态机）
...
> v3 实现（2026-06 升级），代码位于 `MatchingEngine.java`。使用 Jieba 中文分词库进行智能中文分词，支持语义匹配。

### 22.1 文本相似度算法（Jieba 分词）

#### 22.1.1 算法流程

```mermaid
flowchart TD
    A[文本预处理<br/>合并标题+描述+地点<br/>转小写] --> B[Jieba 分词<br/>智能中文分词]
    B --> C[过滤停用词<br/>保留长度≥2的词<br/>或纯英文单词]
    C --> D[词袋去重<br/>HashSet]
    D --> E[关键词匹配计算<br/>精确匹配3分<br/>包含匹配2分]
    E --> F[相似度归一化<br/>取值范围 0-1]
```
...
| "宿舍钥匙带志愿者钥匙扣" | ["宿舍", "钥匙", "带", "志愿者", "钥匙扣"] |
| "苹果手机 iPhone 13" | ["苹果", "手机", "iPhone", "13"] |

**对比 N-gram 方案**：
- N-gram：`["一串", "串钥", "钥匙", "匙有", "有一", "有个", ...]`
- Jieba：`["一串", "钥匙", "有", "一个", "钥匙扣"]` ✅ 无噪音词
...
private BigDecimal calculateChineseSimilarity(String text1, String text2) {
    ...
    // 3. Jieba 分词
    List<String> words1 = jiebaSegmenter.sentenceProcess(left);
    List<String> words2 = jiebaSegmenter.sentenceProcess(right);
    
    // 4. 过滤：长度≥2 或纯英文
    Set<String> set1 = filterWords(words1);
    Set<String> set2 = filterWords(words2);
    
    // 5. 计算匹配分数
    int matchScore = 0;
    for (String w1 : set1) {
        for (String w2 : set2) {
            if (w1.equals(w2)) matchScore += 3;           // 精确匹配
            else if (w1.contains(w2) || w2.contains(w1)) matchScore += 2;  // 包含匹配
        }
    }
    
    // 6. 归一化
    int total = set1.size() + set2.size();
    double score = total > 0 ? (double) matchScore / total : 0;
...
| 维度 | 权重 | 说明 |
|------|------|------|
| TEXT 文本相似度 | 35% | 使用 Jieba 中文分词库，对标题+描述+地点的拼接字符串计算相似度 |
